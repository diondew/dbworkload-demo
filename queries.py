...
class Queries:
    ...
    def txn_0(self, conn: psycopg.Connection):
        ...
        with conn.cursor() as cur:
            # members variables
            self.memid: UUID  = uuid4()
            self.surname: str = random.choice(["Smith","Jones","Fried","Smoltz","Maddux","Avery","Glavine","Cox"])
            self.firstname: str = random.choice(["Chipper","Steve","Greg","John","Lonnie","Tom","Bobby","Max"])
            self.address: str = self.random_str(25)
            self.zipcode: int = random.randint(10000, 99999)
            self.telephone: str = str(random.randint(1000000000, 9999999999)) 
            self.recommendedby: int = random.randint(0,100000)
            self.joindate = dt.datetime.fromtimestamp(random.randint(1655032268, 1759232268))
            cur.execute(
                """
                INSERT INTO cc.members
